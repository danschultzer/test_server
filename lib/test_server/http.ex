defmodule TestServer.HTTP do
  @moduledoc """
  Public API for HTTP and WebSocket mock servers.

  See `TestServer` for full documentation.
  """
  ...
  defp start_with_ex_unit(options, _sup) do
    [_first_module_entry | stacktrace] = InstanceManager.get_stacktrace(__MODULE__)
    caller = self()
    ...
    case InstanceManager.start_instance(caller, Instance.child_spec(options)) do
      {:ok, instance} ->
  ...
  defp raise_start_failure({:error, error}) do
    raise """
    EXIT when starting #{inspect(Instance)}:
  ...
  def get_instance do
    case InstanceManager.fetch_instance(self(), :http, __MODULE__) do
  ...
  defp register_route(instance, uri, options) do
    ...
    [_register_route, _first_module_entry | stacktrace] =
      InstanceManager.get_stacktrace(__MODULE__)
