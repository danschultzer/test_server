...
defmodule TestServer.HTTPServer.Httpd do
  ...
  def start(instance, port, scheme, options, httpd_options) do
    httpd_options =
      ...
      |> Keyword.put_new(:server_name, 'Httpd Test Server')
  ...
  Record.defrecord :httpd, Record.extract(:mod, from_lib: "inets/include/httpd.hrl") ++ [handler_plug: :undefined, websocket: :undefined]
  ...
  defp handle_websocket(%Plug.Conn{adapter: {_adapter, {:websocket, _opts, _data}}}) do
    {:proceed, response: {422, 'WebSocket is not supported with httpd!'}}
  end
