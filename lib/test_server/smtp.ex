defmodule TestServer.SMTP do
  ...
  alias TestServer.SMTP.{Instance, Server}
  ...
  ## Options
    ...
    * `:to`    - a function/1 `fn email -> response end` called when
      the handler matches. Defaults to accepting the email with `:ok`;
    * `:match` - a function/1 `fn email -> boolean end` that receives
      the email and returns a boolean. Defaults to matching any email;

  ## Response format

    * `:ok` — accept the email, responds with "250 2.0.0 Ok"
    * `{:ok, "250 Custom message"}` — accept with custom response
    * `:error` — reject the email, responds with "550 5.0.0 Rejected"
    * `{:error, "550 Custom error"}` — reject with custom response
  ...
  ## Examples
      ...
      # Match on sender
      TestServer.SMTP.add(instance,
        match: fn email -> email.mail_from == "sender@example.com" end,
        to: fn _email -> :ok end
      )

      # Reject email
      TestServer.SMTP.add(instance,
        to: fn _email -> {:error, "550 5.1.1 User unknown"} end
      )
  """
