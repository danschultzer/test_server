defmodule TestServer.SMTP.Instance do
  ...
  def init(options) do
    ...
    options = Keyword.put(options, :tls_options, tls_options)
  ...
  def handle_call({:register, {options, stacktrace}}, _from, state) do
    handler = %{
      ...
      to: Keyword.get(options, :to, &default_handler/1),
  ...
  defp validate_response!(:ok, _stacktrace), do: @default_ok_response

  defp validate_response!({:ok, message}, _stacktrace) when is_binary(message), do: message

  defp validate_response!(:error, _stacktrace), do: @default_error_response

  defp validate_response!({:error, message}, _stacktrace) when is_binary(message), do: message
  ...
  defp validate_response!(response, stacktrace) do
    ...
    Expected one of the following:
      ...
      - :ok
      - {:ok, "250 Custom message"}
      - :error
      - {:error, "550 Custom error"}

    #{Enum.map_join(stacktrace, "\n    ", &Exception.format_stacktrace_entry/1)}
    """
  end

  defp default_handler(_email), do: :ok
end
