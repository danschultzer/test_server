defmodule TestServer.TCPTest do
  ...
  describe "handle/2" do
    ...
    test "with state carried across handlers" do
      {:ok, _instance} = TestServer.TCP.start()
    ...
    test "when receiving unexpected data while a handler is waiting" do
      defmodule UnexpectedDataActiveHandlerTest do
        use ExUnit.Case

        test "fails" do
          {:ok, _instance} = TestServer.TCP.start(suppress_warning: true)
          :ok = TestServer.TCP.handle(match: fn _data, _state -> false end)

          assert {:ok, socket} = unquote(__MODULE__).tcp_connect()
          assert :ok = :gen_tcp.send(socket, "ping")
          assert {:ok, data} = :gen_tcp.recv(socket, 0)
          assert data =~ "received unexpected TCP data"
          assert data =~ "Active handlers:"
        end
      end

      assert io = capture_io(fn -> ExUnit.run() end)
      assert io =~ "received unexpected TCP data"
      assert io =~ "Active handlers:"
    end
    ...
    test "when `:to` function returns an invalid response" do
    ...
    test "when sending the reply fails" do
      defmodule ReplySendFailsTest do
        use ExUnit.Case

        test "fails" do
          {:ok, _instance} = TestServer.TCP.start(suppress_warning: true)

          :ok = TestServer.TCP.handle(to: fn _data, state -> {:reply, :not_iodata, state} end)

          assert {:ok, socket} = unquote(__MODULE__).tcp_connect()
          assert :ok = :gen_tcp.send(socket, "ping")
          assert {:ok, data} = :gen_tcp.recv(socket, 0)
          assert data =~ "TCP reply failed, because:"
        end
      end

      assert io = capture_io(fn -> capture_log(fn -> ExUnit.run() end) end)
      assert io =~ "TCP reply failed, because:"
    end
  end
  ...
  describe "connect/2" do
    ...
    test "with multiple clients, pairs them to connections in order" do
      {:ok, _instance} = TestServer.TCP.start()
    ...
    test "with a handler set on one connection" do
      {:ok, _instance} = TestServer.TCP.start()
    ...
    test "with a client that connects without a registered connection" do
      {:ok, _instance} = TestServer.TCP.start()
  ...
  describe "send/2" do
    ...
    test "with multiple clients, sends to only one" do
      {:ok, _instance} = TestServer.TCP.start()
      {:ok, conn_1} = TestServer.TCP.connect()
      {:ok, conn_2} = TestServer.TCP.connect()

      socket_1 = connect_bound(conn_1)
      socket_2 = connect_bound(conn_2)
    ...
    test "with state carried across sends and handlers" do
      {:ok, _instance} = TestServer.TCP.start()
      {:ok, conn} = TestServer.TCP.connect()
    ...
    test "with invalid callback response" do
      defmodule SendInvalidResponseTest do
        ...
        test "fails" do
          {:ok, _instance} = TestServer.TCP.start(suppress_warning: true)
          {:ok, conn} = TestServer.TCP.connect()
          socket = unquote(__MODULE__).connect_bound(conn)
  ...
  def connect_bound(conn) do
    :ok = TestServer.TCP.handle(conn, to: fn data, state -> {:reply, data, state} end)

    {:ok, socket} = tcp_connect()
    :ok = :gen_tcp.send(socket, "__bind__")
    {:ok, "__bind__"} = :gen_tcp.recv(socket, 0)
